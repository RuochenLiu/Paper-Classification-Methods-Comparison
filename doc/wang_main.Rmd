...
## Step 4: Evaluation 
Here, we still use A Gputa test data to evaluate how the algorithm perform following the measurement in paper 1.
```{r}
source("../Spr2017-proj4-team-14/lib/evaluation_measures.R")
result <- matching_matrix(G = test_1[,1], prediction)
perfomance <- performance_statistics(result_matrix = result)

compare_df <- data.frame(method="Coauthor",
                         precision=perfomance$precision,
                         recall=perfomance$recall,
                         f1=perfomance$f1,
                         accuracy=perfomance$accuracy)

kable( compare_df, digits = 2)
```